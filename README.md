...
`pi-life` reads `profiles/<life>.yaml` and execs `pi --no-skills` plus `--skill` for each allowlisted mantra/pack path under `~/.agents/skills`. Missing packs warn and still launch. Missing mantra or tracker paths, or invalid YAML, exits 2. `tracker: none` (or omitting tracker) loads no tracker skill. `chain`/`team` currently use the same skill allowlist as `solo` until #6/#8.
