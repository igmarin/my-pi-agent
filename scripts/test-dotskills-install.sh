#!/usr/bin/env bash
# Real installer -> pi-life allowlist, with local Git fixture sources and a temp home.
set -euo pipefail
...
(
  cd "$tmp/target"
  PI_SKILLS_HOME="$tmp/home/.agents/skills" MY_PI_AGENT_HOME="$tmp/harness" "$root/bin/pi-life" --dry-run ruby > "$tmp/launch.log" 2> "$tmp/launch.err"
)
...
status=0
(cd "$tmp/target" && PI_SKILLS_HOME="$tmp/home/.agents/skills" MY_PI_AGENT_HOME="$tmp/harness" "$root/bin/pi-life" --dry-run ruby) > "$tmp/broken.log" 2>&1 || status=$?
[[ "$status" -eq 2 ]]
