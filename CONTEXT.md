...
**Doctor**:
`pi-life doctor [life]` health check (replaces the pre-#13 stub). Fail-closed (exit 2): `pi`, `bun`, overlay parse failure, malformed profile/manifest (both modes), and — for the given `[life]` — a missing required mantra or tracker skill path (the same contract the launcher enforces; mantra skills are user-provisioned under `PI_SKILLS_HOME`/`~/.agents/skills`, never vendored). With no `[life]`, doctor sweeps every profile and warns (deduped, exit 0) on missing mantra/tracker/pack skills — advisory, since users may run a subset of lives. Warns and exits 0: optional gaps (missing packs for the given `[life]`, `just`, `rs-guard`, `herdr`, `core.excludesfile` patterns). Prints the report keys `life`/`harness`/`cwd`/`overlay`/`required`/`optional`. Diagnostics (warnings, parse errors) go to stderr; the report goes to stdout.
