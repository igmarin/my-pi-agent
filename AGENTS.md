...
- Profiles: YAML at `profiles/<vida>.yaml`. Launch is `pi -e extensions/damage-control-continue.ts --no-skills` then `--skill` for each allowlisted mantra, pack, and tracker — the INV-skills contract (invariant: damage-control gate + `--no-skills` + only allowlisted `--skill` paths).
...
- Herdr is a host: `herdr agent start <name> --kind pi -- pi-vida <vida>`. Do not wrap Herdr in a Pi extension.
