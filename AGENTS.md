...
- Memory: `extensions/memory.ts` + `memoryHelpers.ts`; store at `PI_MEMORY_HOME` or `~/.agents/memory` (markdown). Primary writes, children read-only. Never inside a repo; never shell out to Herdr for scope.
