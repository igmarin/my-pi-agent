...
- Extensions: `export default function (pi)`. Skip `ctx.ui` when `!ctx.hasUI`. Stacked `-e`: first extension wins the theme. `cross-agent` / `system-select` live under `extensions/` but are not passed by `pi-life` yet. `fusion` loads the vendored `extensions/fusion-harness/` (MIT, disler/fusion-harness @ `01a3482`, own `LICENSE` + `tests/`): `pi-life <life> fusion <stack.yaml>` passes `--fh-config`; the stack's `primary` slot becomes the host model, so solo `--model`/`--thinking` are dropped with a warning. `stacks/` holds copy templates (`model-stack-trio.yaml`; `model-stack-cognition.yaml` — Cognition SWE slots, provider setup in docs/how-to.md; a `cognition/` slot without `COGNITION_API_KEY` warns, never fails). Agent search: `profiles/<life>/agents/` (YAML), shared `profiles/agents/`, cwd `.pi/agents/`, then `.claude/.gemini/.codex` (cwd), then `~/.claude/.gemini/.codex`. First-wins on name. Alias `rails` → `ruby`.
