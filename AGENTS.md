...
- `chain` / `team` print “not wired yet (#6/#8)” and use the solo allowlist. `pi-life doctor [life]` checks required launcher pieces (`pi`, `bun`) fail-closed, warns on optional gaps (packs, `just`, `rs-guard`, excludesfile), and prints the resolved overlay.
...
Overlay merge (#11), chain (#6), team (#8), boot TUI / model policy (#15).
